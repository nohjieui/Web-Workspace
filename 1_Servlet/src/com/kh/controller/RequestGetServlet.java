...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * - request.getParameterValue("키값") : String[](그에 해당하는 value값)
		 * => 하나의 key값으로 여러개의 value값을 받는 경우(체크박스) 문자열 배열형으로 반환가능
		 */
		String name = request.getParameter("name"); // 텍스트박스가 비어있는 겨우 빈문자열("")로 넘어온다
		String gender = request.getParameter("gender"); // 라디오버튼의 경우 체크된 값이 없을 때 빈 문자열이 아닌 null이 넘어간다
		...
		if(foods == null) {
			...
			System.out.println("foods : "+ String.join(",", foods)); // food.join(","); -> 배열에 있는 모든 값들을 구분자를 통해서 하나의 문자열로 반환해주는 메소드 -> String에 내장되어있는 메소드
		}
		 ...
		 * 
		 * 단점 : 복잡, 혹시라도 나중에 html수정해야할 때 Java코드 내에서 수정이 이루어지기 때문에
		 * 		 수정된 내용을 다시 반영시키고자 한다면 서버를 재실행해야한다.
		 * 		 사소한 오타로 에러날 수 있다.
		 */
		...
		out.println("<html>");
			...
			out.println("<body>");
				...
				if(gender == null) {
					...
					out.print("<span id='gender'>"+(gender.equals("M") ? "남자":"여자")+"</span>입니다<br>"); // 3항연산자사용
				}
				...
				if(foods == null) {
					...
					out.print("<ul>");
						...
						// out.printf("<li>"+String.join(",",foods)+"</li>"); -> 이렇게도 작성가능
					out.print("</ul>");
				}
			out.println("</body>");
		out.println("</html>");
		...
		// response.getWriter().append("Served at: ").append(request.getContextPath()); -> 원래있던 코드로 주석처리함(아니면 이게 실행되서 아무것도 출력이 안됨)
	}
